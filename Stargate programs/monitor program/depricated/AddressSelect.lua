...
local twiforest = {8,16,7,25,19,30,0}
local miningdim = {16,7,9,29,5,1,0}
local everbright= {26,28,13,33,7,35,0}
...
function DisconnectCheck() -- does exactly what you think it does (if an unexpected error is thrown this siezes the program too)
    ...
    if (disCode ~= 7 or 8 or 9 or 10 or -1 or -15 or -16 or -19) then
        while true do
            print(disCode)
        end
    end
...
function ParaDial() -- seperating touch dialing so timeout function can work
    while dialing == false do
        ...
        --     dialing = true
        if (y == 2) and ((x >= 11) and (x <= 18)) then
            Dial(theNether)
            destAddress = theNether
            dialing = true
        ...
        elseif (y == 6) and ((x >= 2) and (x <= 9)) then
            Dial(theOther)
            destAddress = theOther
            dialing = true
        elseif (y == 6) and ((x >= 11) and (x <= 18)) then
            Dial(theBeyond)
            destAddress = theBeyond
            dialing = true
        elseif (y == 6) and ((x >= 20) and (x <= 27)) then
            Dial(theAether)
            destAddress = theAether
            dialing = true
        elseif (y == 8) and ((x >= 2) and (x <= 9)) then
            Dial(voidscape)
            destAddress = voidscape
            dialing = true
        elseif (y == 8) and ((x >= 11) and (x <= 18)) then
            Dial(twiforest)
            destAddress = twiforest
            dialing = true
        elseif (y == 8) and ((x >= 20) and (x <= 27)) then
            Dial(miningdim)
            destAddress = miningdim
            dialing = true
        elseif (y == 2) and ((x >= 2) and (x <= 9)) then
            Dial(everbright)
            destAddress = everbright
            dialing = true
        end
    end
    return 1
...
function PRIMARYDialingOut() -- what it says
    ...
    x, y = GetClick()
    local PDO = 0
    PDO = parallel.waitForAny(ParaDial, Paratimeout)
